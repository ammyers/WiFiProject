...
public class Sender implements Runnable {
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				// If there are Packets to be sent in the LinkLayer's outgoing
				// queue block
				// and checks the network for activity
				if (!theLink.getOutgoingBlock().isEmpty()) {
					int counter = 0;
					Packet packet = new Packet(theLink.getOutgoingBlock()
							.take().getFrame());
						...
						while (counter < theRF.dot11RetryLimit) {
							if (!theRF.inUse()) {
								...
								if (!theRF.inUse()) {
									...
									// Was idle, we waited IFS, then wasn't idle
									// anymore
								} else {
									waitBS();
									expBackoff();
	...
	/**
	 * Wait even longer. We can transmit right away if the network isn't still
	 * busy after the wait.
	 */
	...
	/**
	 * Wait for Ack. If no ack is received, increase collision window and set
	 * retry bit in packet to 1.
	 *
	 * @return true if received, false if not received within reasonable time.
	 */
