...
public class Receiver implements Runnable {
	...
	public void run() {
		...
		while (true) {
			...
			if (!theLink.getIncomingBlock().isEmpty()) {
				...
				if (destAddr == theLink.ourMAC) {
					...
					// if the destination address is ours and the packet is a
					// data packet
					if (packet.getFrameType() == 0) {

						short nextSeqNum = theLink.gotSeqNum(packet
								.getSenderAddr());

						// if the packets sequence number is larger than the
						// expected one
						if (packet.getSeqNum() > nextSeqNum) {
							theLink.output
							.println("Packet Sequence out of order, expected: "
									+ nextSeqNum
									+ " got: "
									+ packet.getSeqNum());
						}
						try {
							// Puts the new Packet into the LinkLayer's inbound
							// queue
						...
						Packet ack = new Packet(1, packet.getSeqNum(),
								packet.getSenderAddr(), theLink.ourMAC, null);
